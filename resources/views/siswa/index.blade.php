<x-app-layout>
    ...
    <div class="flex items-center gap-2 text-sm text-gray-500 mb-4">
        ...
        <span>›</span>
        <span class="text-[#03045E] font-bold">Manajemen Siswa</span>
    </div>
    ...
    <!-- Search & Filter -->
    <form method="GET" action="{{ route('siswa.index') }}" class="flex gap-3 mb-6">
        ...
        <select name="kelas" class="rounded-full border-gray-200 px-6 py-3 focus:ring-[#03045E] focus:border-[#03045E] shadow-sm">
            <option value="">Semua Kelas</option>
            @foreach($kelas as $item)
                <option value="{{ $item->id }}" {{ request('kelas') == $item->id ? 'selected' : '' }}>
                    {{ strtoupper(str_replace('-', ' ', $item->nama_kelas)) }}
                </option>
            @endforeach
        </select>
        ...
        @if(request('search') || request('kelas'))
            <a href="{{ route('siswa.index') }}" class="px-6 py-3 bg-gray-200 text-gray-700 rounded-full font-bold shadow-sm hover:scale-105 transition-all">
                Reset
            </a>
        @endif
    </form>
    ...
    @php
        $currentSort = request()->input('sort', 'nisn');
        ...
        function getSortUrl($column, $currentSort, $currentDirection) {
            ...
            return route('siswa.index', $params);
        }
        ...
        function getSortIcon($column, $currentSort, $currentDirection) {
            ...
            return '<svg class="w-4 h-4 inline ml-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">' . $icon . '</svg>';
        }
    @endphp
    ...
    <div id="modal-hapus" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/50">
        <div class="bg-white rounded-2xl shadow-xl p-8 max-w-sm w-full mx-4">
            <div class="flex flex-col items-center text-center gap-4">
                ...
                <div>
                    <h3 class="text-lg font-black text-[#03045E]">Hapus Siswa?</h3>
                    <p class="text-gray-500 text-sm mt-1">
                        Data siswa <span id="nama-siswa" class="font-bold text-[#03045E]"></span> 
                        dengan NISN <span id="nisn-siswa" class="font-bold text-[#03045E]"></span> 
    ...
    <script>
        function bukaModal(url, nama, nisn) {
            document.getElementById('nama-siswa').textContent = nama;
            document.getElementById('nisn-siswa').textContent = nisn;
