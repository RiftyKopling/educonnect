<x-app-layout>
    ...
    <div class="mb-6 flex justify-between items-center flex-wrap gap-3">
        <div>
            ...
            <p class="text-gray-500 text-sm mt-1">Melihat daftar riwayat presensi siswa per mata pelajaran.</p>
        </div>
                ...
                <tbody class="text-[#03045E] font-medium">
                    ...
                    <tr class="bg-gray-50 hover:bg-gray-100 transition-all">
                        <td class="p-4 rounded-l-2xl">
                            <span class="px-3 py-1 bg-[#03045E]/10 rounded-xl text-sm font-bold whitespace-nowrap">
                                {{ \Carbon\Carbon::parse($p->tanggal)->translatedFormat('d M Y') }}
                            </span>
                        </td>
                        <td class="p-4">
                            <div class="font-bold">{{ $p->siswa->nama_lengkap ?? '-' }}</div>
                            <div class="text-xs text-gray-500 font-normal">NISN: {{ $p->siswa_nisn }}</div>
                        </td>
                        <td class="p-4">
                            <div class="font-bold">{{ $p->mapel->nama_mapel ?? '-' }}</div>
                            <div class="text-xs text-gray-500 font-normal">Kelas: {{ $p->kelas->nama_kelas ?? '-' }}</div>
                        </td>
                        <td class="p-4 text-center">
                            @if($p->status == 'H')
                                <span class="px-3 py-1 bg-emerald-100 text-emerald-700 rounded-xl text-sm font-bold">Hadir</span>
                            @elseif($p->status == 'S')
                                <span class="px-3 py-1 bg-amber-100 text-amber-700 rounded-xl text-sm font-bold">Sakit</span>
                            @elseif($p->status == 'I')
                                <span class="px-3 py-1 bg-blue-100 text-blue-700 rounded-xl text-sm font-bold">Izin</span>
                            @elseif($p->status == 'A')
                                <span class="px-3 py-1 bg-red-100 text-red-700 rounded-xl text-sm font-bold">Alpa</span>
                            @elseif($p->status == 'D')
                                <span class="px-3 py-1 bg-purple-100 text-purple-700 rounded-xl text-sm font-bold">Dispensasi</span>
                            @endif
                        </td>
                            ...
                            <div class="flex justify-center gap-2">
                                ...
                                @if(auth()->user()->hasRole('guru-mapel'))
                                    <button type="button"
                                        ...
                                        data-nama="{{ $p->siswa->nama_lengkap ?? 'Siswa' }} - {{ $p->mapel->nama_mapel ?? '' }} - {{ \Carbon\Carbon::parse($p->tanggal)->translatedFormat('d M Y') }}"
                    ...
                    <tr>
                        <td colspan="5" class="p-8 text-center text-gray-400">
                            <div class="flex flex-col items-center gap-2">
                                ...
                                <span class="font-bold text-gray-400">Belum ada data presensi</span>
                                @if(request('search') || request('status') || request('kelas_id') || request('tanggal'))
                                    <span class="text-sm text-gray-400">Coba ubah filter pencarian</span>
                                    <a href="{{ route('presensi.index') }}" class="mt-2 px-4 py-2 bg-gray-100 text-gray-600 rounded-full text-sm font-bold hover:bg-gray-200 transition-all">Reset Pencarian</a>
                                @endif
                            </div>
                        </td>
                    </tr>
