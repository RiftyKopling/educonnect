    ...
    <div class="mb-6 flex justify-between items-center">
        <div>
            ...
            <p class="text-gray-500">
                Mata Pelajaran: <span class="font-bold text-[#03045E]">{{ $mapel->nama_mapel }}</span>
        ...
        <form action="{{ route('mapel.storeAssign', $mapel->id) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
                @forelse($gurus as $guru)
                    ...
                    @endphp
                    <label class="flex items-center p-4 border-2 rounded-2xl cursor-pointer transition-all hover:shadow-md {{ $isChecked ? 'border-[#03045E] bg-[#03045E]/5' : 'border-gray-200 hover:border-[#03045E]/30' }}">
                        <input type="checkbox" 
                               name="guru_ids[]" 
                               value="{{ $guru->id }}" 
                               class="w-5 h-5 text-[#03045E] rounded border-gray-300 focus:ring-[#03045E] focus:ring-2"
                               {{ $isChecked ? 'checked' : '' }}>
                        <div class="ml-3">
